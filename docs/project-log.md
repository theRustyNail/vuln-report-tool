...
Designed the evaluation approach and settled the classification handling for the upcoming evaluation script (vulnreport/evaluate.py). The module itself is not yet written. The plan is for it to run each scan through the existing parse and match pipeline and score the result against a separate answer key. It will be a detection metric, not a scoring one, needing no NVD calls and running offline. I have settled the classification handling: precision and recall will be counted only over targets where the tool commits to a direct match; tool_decided and not-yet-run targets will get their own rows instead of being forced into binary bins; nginx is out, and vsftpd is deferred to the EMA. Computing the metrics by hand on the current state yields a snapshot of three true positives (2.4.49, 2.4.50, OpenSSH) and one false negative (Samba). This manual calculation gives a precision of 1.00 and a recall of 0.75, which the script will eventually automate.
